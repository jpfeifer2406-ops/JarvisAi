    ...
    if mode == "stt":
        ...
        if key not in _MODELS:
            _MODELS[key] = WhisperModel(
                ...
                device="cpu",
                compute_type="int8",
                cpu_threads=int(os.environ.get("COMPUTER_CPU_THREADS", "2")),
                num_workers=1,
                local_files_only=True,
            )
        segments, _ = _MODELS[key].transcribe(
            payload["input"], language="de", beam_size=1, vad_filter=True, condition_on_previous_text=False
        )
        ...
        with torch.inference_mode():
            if payload.get("reference"):
                ...
                wavs, sr = model.generate_voice_clone(
                    text=payload["text"], language=p["language"], voice_clone_prompt=_PROMPTS[prompt_key]
                )
            else:
                wavs, sr = model.generate_voice_design(
                    text=payload["text"], language=p["language"], instruct=p["description"]
                )

...
class Pending:
    ...
    def public(self):
        return {
            ...
            "expires_in": max(0, round(self.expires - time.monotonic())),
...
class Broker:
    ...
    async def decide(self, session_id: str, action_id: str, digest: str, phrase: str, approve: bool):
        async with self.lock:
            ...
            if (
                ...
                or action.future.done()
                or action.expires <= time.monotonic()
                or action.digest != digest
            ):
    ...
    def list_for(self, session_id):
        return [
            a.public() for a in self.pending.values() if a.session_id == session_id and not a.future.done()
        ]

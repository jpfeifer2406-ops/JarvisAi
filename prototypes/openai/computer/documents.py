...
class Documents:
    ...
    def render(text: str, kind: str) -> bytes:
        ...
        if kind == "docx":
            ...
            flow = [Paragraph(escape(line) or "&#160;", styles["Normal"])
                    for line in text.split("\n")]
    ...
    def create(self, title: str, text: str, kind: str, parent: str | None = None):
        ...
        meta = {"id": doc_id, "title": title, "format": kind, "parent": parent,
                "sha256": hashlib.sha256(blob).hexdigest(),
                "text_sha256": hashlib.sha256(text.encode()).hexdigest(), "characters": len(text)}
        try:
            for name, data in (("document."+kind, blob), ("source.txt", text.encode()),
    ...
    def read(self, doc_id: str):
        ...
        if (hashlib.sha256(blob).hexdigest() != meta["sha256"] or
                hashlib.sha256(text.encode()).hexdigest() != meta["text_sha256"]):
    ...
    def list(self):
        return [self.read(p.name)[0] for p in self.root.iterdir()
                if re.fullmatch(r"[a-f0-9]{32}", p.name)][:200]
